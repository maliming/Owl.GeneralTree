    ...
    where TDbContext : IAbpMongoDbContext
{
    ...
    protected virtual async Task<FilterDefinition<TTree>> CreateEqualIdFilterAsync(TPrimaryKey id, bool applyFilters = false)
    {
        ...
        if (applyFilters)
        {
            await RepositoryFilterer.AddGlobalFiltersAsync(filters);
        }

        return Builders<TTree>.Filter.And(filters);
    }

    protected virtual async Task<FilterDefinition<TTree>> CreateNotEqualIdFilterAsync(TPrimaryKey id, bool applyFilters = false)
    {
        var filters = new List<FilterDefinition<TTree>>
        {
    ...
    protected virtual async Task<FilterDefinition<TTree>> CreateEqualParentIdFilterAsync(TPrimaryKey id, bool applyFilters = false)
    {
        var filters = new List<FilterDefinition<TTree>>
        {
    ...
    protected virtual async Task<FilterDefinition<TTree>> CreateNotEqualParentIdFilterAsync(TPrimaryKey id, bool applyFilters = false)
    {
        var filters = new List<FilterDefinition<TTree>>
        {
